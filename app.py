# import time, uuid, json, shutil, subprocess, logging, os
...
#         else:
#             # Step 1: 生成交易
#             unsigned_file = f"{file_prefix}-unsigned.json"
#             exit_code, stdout_raw, stderr_raw = run_cmd(base_cmd + ["--generate-only"])
#             stdout_raw = stdout_raw.strip()
#             if exit_code != 0:
#                 result = {
#                     "success": False,
#                     "command": " ".join(base_cmd),
...
#                 logger.info(f"Result: {result}")
#                 return result
#             with open(unsigned_file, "w", encoding="utf-8") as f:
#                 f.write(stdout_raw)
#             temp_files.append(unsigned_file)
#             logger.info("Generated unsigned tx: %s", " ".join(base_cmd + ["--generate-only"]))
#
#             # Step 2: 多签签名
#             signed_files = []
#             for signer in multisig_signers:
#                 signed_file = f"{file_prefix}-signed-{signer}.json"
#                 cmd_sign = [
#                     bin_path, "tx", "sign", unsigned_file,
#                     "--from", signer,
...
#                 logger.info("Signed with %s: %s", signer, " ".join(cmd_sign))
#
#             # Step 3: 合并签名
#             multisigned_file = f"{file_prefix}-multisigned.json"
#             cmd_multisign = [
#                 bin_path, "tx", "multisign", unsigned_file, multisig_name
#             ] + signed_files + [
#                 "--chain-id", chain_id,
#                 "--node", node,
#                 "--home", home,
...
#                 logger.info(f"Result: {result}")
#                 return result
#             with open(multisigned_file, "w", encoding="utf-8") as f:
#                 f.write(stdout_raw)
#             temp_files.append(multisigned_file)
#             logger.info("Merged signatures: %s", " ".join(cmd_multisign))
#
#             # Step 4: 广播交易
#             cmd_broadcast = [bin_path, "tx", "broadcast", multisigned_file,
#                              "--chain-id", chain_id,
#                              "--node", node,
#                              "--home", home,
#                              "--keyring-backend", keyring_backend,
...
#                 "stderr": process_output(stderr_raw),
#             }
#             logger.info(f"Broadcast tx: {result}")
#
#     finally:
#         for f in temp_files:
#             try:
...
#     except Exception as e:
#         logger.exception("Error reading jsonl log file: %s", e)
...
#             rec = json.loads(ln)
#         except json.JSONDecodeError:
#             logger.warning("Skipping invalid json log line: %s", ln[:200])
#             continue
